package com.p1nero.lmm.client.model;
...
public class ModelNewWolf<T extends LivingEntity> extends Model<T> {
    ...
    @Override
    public void prepareMobModel(@NotNull T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTickTime) {
    ...
    @Override
    public void setupAnim(@NotNull T entity, float limbSwing, float limbSwingAmount, float tailRotation, float netHeadYaw, float headPitch) {
        this.headPitch(head, headPitch);
        this.headYaw(head, netHeadYaw);
        tail01.xRot = 0.6981317007977318F + tailRotation;
        boolean sit = false;

        if(entity instanceof XiaoTian wolf) {
