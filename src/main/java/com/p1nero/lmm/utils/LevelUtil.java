...
public class LevelUtil {
    ...
    public static boolean circleSlamFracture(@Nullable LivingEntity caster, Level level, Vec3 center, double radius, boolean noSound, boolean noParticle, boolean hurtEntities) {
        ...
        if (!level.isClientSide) {
            NetworkManager.sendToAllPlayerTrackingThisChunkWithSelf(new SPFracture(center, radius, noSound, noParticle), level.getChunkAt(blockPos));
        }
...
//                            ,damage);
            ...
            if (!noSound) {
                level.playLocalSound(center.x, center.y, center.z, smallSlam ? LMMSounds.GROUND_SLAM_SMALL.get() : LMMSounds.GROUND_SLAM.get(), SoundSource.BLOCKS, 1.0F, 1.0F, false);
            }

            if (!smallSlam && !noParticle) {
                level.addParticle(LMMParticles.GROUND_SLAM.get(), center.x, center.y, center.z, 1.0D, radius * 10.0D, 0.5D);
            }
        }
